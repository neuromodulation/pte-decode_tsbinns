...
def violinplot_results(
    ...
    if colours is not None:
        ...
        palette = {item: colour for item, colour in zip(order, colours)}
    else:
        palette = None
...
def lineplot_prediction_compare(
    ...
    ax.set_ylabel(y_label)
    ax.legend(handlelength=1, handletextpad=0.4)
    if y_lims:
        ax.set_ylim(y_lims[0], y_lims[1])
    fig.tight_layout()
    if outpath is not None:
...
def _pval_correction_lineplot(
    ...
    if correction_method == "cluster":
        ...
        correction_method in ["cluster_pvals", "fdr"]
        or correction_method is None
    ):
